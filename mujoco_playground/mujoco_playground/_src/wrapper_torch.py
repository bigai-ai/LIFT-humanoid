...
class RSLRLBraxWrapper(VecEnv):
    ...
    def step(self, action):
        ...
        if type(info["first_obs"]) == dict:
            info_ret["observations"]["raw"] = {
              "obs": _jax_to_torch(info["first_obs"]["state"]),
              "critic_obs": _jax_to_torch(info["first_obs"]["privileged_state"]) if self.asymmetric_obs else None
            }
        else:
            info_ret["observations"]["raw"] = {
              "obs": _jax_to_torch(info["first_obs"]),
              "critic_obs": None,
            }
