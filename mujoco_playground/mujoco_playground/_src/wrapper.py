...
def wrap_for_brax_evaluating(  
    env: mjx_env.MjxEnv,
    obs_history_length:int = 1, #TODO: use this to add history to the observation
    priv_obs_history_length: int = 1, #TODO: use this to add history to the private observation
    vision: bool = False,
    ...
    env = brax_training.EpisodeNoResetWrapper(env, episode_length, action_repeat)
    return env
...
class BraxAutoResetWrapper(Wrapper):
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        # if rew_info is the property of the state
        if hasattr(state, 'rew_info'):
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        if hasattr(state, 'rew_info'):
            ...
            state = state.replace(data=data, obs=obs, rew_info=rew_info)
        else:
