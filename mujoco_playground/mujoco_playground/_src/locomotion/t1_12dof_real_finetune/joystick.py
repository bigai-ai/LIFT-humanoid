...
def default_config() -> config_dict.ConfigDict:
    return config_dict.create(
        ...
        reward_config=BoosterUtils.reward_config(),
...
class Joystick(t1_base.T1LowDimEnv):
    ...
    def __init__(
        ...
        self.obs_limits = BoosterUtils.wm_state_limits
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        obs['wm_state'] = jp.tile(obs['wm_state'], self._config.wm_obs_history_length)
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        wm_obs_stack = jp.concatenate(
          [obs['wm_state'], state.obs['wm_state'][:obs['wm_state'].shape[0]*(self._config.wm_obs_history_length-1)]],
          axis=-1
        )
    ...
    def _get_obs(
        ...
        actor_state = actor_state.at[BoosterUtils.actor_gravity_idxs].set(noisy_gravity)
        actor_state = actor_state.at[BoosterUtils.actor_base_ang_vel_idxs].set(noisy_gyro)
        actor_state = actor_state.at[BoosterUtils.actor_q_idxs].set(noisy_joint_angles)
        actor_state = actor_state.at[BoosterUtils.actor_qd_idxs].set(noisy_joint_vel)
        actor_state = actor_state.at[BoosterUtils.actor_commands_idxs].set(info["command"])
        actor_state = actor_state.at[BoosterUtils.actor_cos_phase_idx].set(phase[0])
        actor_state = actor_state.at[BoosterUtils.actor_sin_phase_idx].set(phase[1])
        actor_state = actor_state.at[BoosterUtils.actor_last_action_idxs].set(info["last_act"])
        ...
        p_f = data.site_xpos[self._feet_site_id]
        p_fz = p_f[..., -1]
        p_fz_min = jp.min(p_fz)
        root_height_offset = root_height #- p_fz_min
        privileged_state = jp.zeros((BoosterUtils.priv_observation_size,))
        privileged_state = privileged_state.at[BoosterUtils.priv_gravity_idxs].set(gravity)
        privileged_state = privileged_state.at[BoosterUtils.priv_base_ang_vel_idxs].set(gyro)
        privileged_state = privileged_state.at[BoosterUtils.priv_q_idxs].set(joint_angles)
        privileged_state = privileged_state.at[BoosterUtils.priv_qd_idxs].set(joint_vel)
        privileged_state = privileged_state.at[BoosterUtils.priv_commands_idxs].set(info["command"])
        privileged_state = privileged_state.at[BoosterUtils.priv_cos_phase_idx].set(phase[0])
        privileged_state = privileged_state.at[BoosterUtils.priv_sin_phase_idx].set(phase[1])
        privileged_state = privileged_state.at[BoosterUtils.priv_last_action_idxs].set(info["last_act"])
        privileged_state = privileged_state.at[BoosterUtils.priv_quat_idxs].set(quat)
        privileged_state = privileged_state.at[BoosterUtils.priv_base_lin_vel_idxs].set(linvel)
        privileged_state = privileged_state.at[BoosterUtils.priv_h_idx].set(root_height_offset)

        wm_state = jp.zeros((BoosterUtils.wm_observation_size,))
        wm_state = wm_state.at[BoosterUtils.wm_gravity_idxs].set(gravity)
        wm_state = wm_state.at[BoosterUtils.wm_base_ang_vel_idxs].set(gyro)
        wm_state = wm_state.at[BoosterUtils.wm_q_idxs].set(joint_angles)
        wm_state = wm_state.at[BoosterUtils.wm_qd_idxs].set(joint_vel)
        wm_state = wm_state.at[BoosterUtils.wm_commands_idxs].set(info["command"])
        wm_state = wm_state.at[BoosterUtils.wm_cos_phase_idx].set(phase[0])
        wm_state = wm_state.at[BoosterUtils.wm_sin_phase_idx].set(phase[1])
        wm_state = wm_state.at[BoosterUtils.wm_last_action_idxs].set(info["last_act"])
        wm_state = wm_state.at[BoosterUtils.wm_quat_idxs].set(quat)
        wm_state = wm_state.at[BoosterUtils.wm_base_lin_vel_idxs].set(linvel)
        wm_state = wm_state.at[BoosterUtils.wm_h_idx].set(root_height_offset)
        wm_state = wm_state.at[BoosterUtils.wm_gait_process_idx].set(info["gait_process"])
        wm_state = wm_state.at[BoosterUtils.wm_gait_frequency_idx].set(info["gait_freq"])

        norm_actor_state = BoosterUtils.normalize_state(
            actor_state, BoosterUtils.actor_state_limits
        )
        norm_privileged_state = BoosterUtils.normalize_state(
            privileged_state, BoosterUtils.priv_state_limits
        )
        norm_wm_state = BoosterUtils.normalize_state(
            wm_state, BoosterUtils.wm_state_limits
        )
