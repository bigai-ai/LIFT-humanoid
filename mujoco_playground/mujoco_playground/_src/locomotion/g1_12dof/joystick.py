...
def default_config() -> config_dict.ConfigDict:
    return config_dict.create(
        ...
        reward_config=g1Utils.reward_config,
...
class Joystick(g1_base.G1Env):
    ...
    def _post_init(self) -> None:
        ...
        self._soft_lowers = g1Utils.soft_lowers
        self._soft_uppers = g1Utils.soft_uppers
        ...
        self._track_body_id = np.array(
            [self._mj_model.body(name).id for name in consts.TRACK_BODIES]
        )
        ...
        track_link_linvel_sensor_adr = []
        for site in consts.TRACK_BODIES:
            sensor_id = self._mj_model.sensor(f"{site}_global_linvel").id
            sensor_adr = self._mj_model.sensor_adr[sensor_id]
            sensor_dim = self._mj_model.sensor_dim[sensor_id]
            track_link_linvel_sensor_adr.append(
                list(range(sensor_adr, sensor_adr + sensor_dim))
            )
        self._track_link_linvel_sensor_adr = jp.array(track_link_linvel_sensor_adr)

        track_link_angvel_sensor_adr = []
        for site in consts.TRACK_BODIES:
            sensor_id = self._mj_model.sensor(f"{site}_global_angvel").id
            sensor_adr = self._mj_model.sensor_adr[sensor_id]
            sensor_dim = self._mj_model.sensor_dim[sensor_id]
            track_link_angvel_sensor_adr.append(
                list(range(sensor_adr, sensor_adr + sensor_dim))
            )
        self._track_link_angvel_sensor_adr = jp.array(track_link_angvel_sensor_adr)
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        motor_targets = self._default_pose + action * g1Utils.action_scale
        data = mjx_env.step(
    ...
    def _get_obs(
        ...
        actor_state = actor_state.at[g1Utils.actor_gravity_idxs].set(jax.lax.stop_gradient(noisy_gravity))
        actor_state = actor_state.at[g1Utils.actor_base_ang_vel_idxs].set(jax.lax.stop_gradient(noisy_gyro))
        actor_state = actor_state.at[g1Utils.actor_q_idxs].set(jax.lax.stop_gradient(noisy_joint_angles))
        actor_state = actor_state.at[g1Utils.actor_qd_idxs].set(jax.lax.stop_gradient(noisy_joint_vel))
        actor_state = actor_state.at[g1Utils.actor_commands_idxs].set(jax.lax.stop_gradient(info["command"]))
        ...
        actor_state = actor_state.at[g1Utils.actor_last_action_idxs].set(jax.lax.stop_gradient(info["last_act"]))

        privileged_state = jp.zeros((g1Utils.priv_observation_size,))
        privileged_state = privileged_state.at[g1Utils.priv_gravity_idxs].set(jax.lax.stop_gradient(gravity))
        privileged_state = privileged_state.at[g1Utils.priv_base_ang_vel_idxs].set(jax.lax.stop_gradient(gyro))
        privileged_state = privileged_state.at[g1Utils.priv_q_idxs].set(jax.lax.stop_gradient(joint_angles))
        privileged_state = privileged_state.at[g1Utils.priv_qd_idxs].set(jax.lax.stop_gradient(joint_vel))
        privileged_state = privileged_state.at[g1Utils.priv_commands_idxs].set(jax.lax.stop_gradient(info["command"]))
        ...
        privileged_state = privileged_state.at[g1Utils.priv_last_action_idxs].set(jax.lax.stop_gradient(info["last_act"]))
        privileged_state = privileged_state.at[g1Utils.priv_quat_idxs].set(jax.lax.stop_gradient(quat))
        privileged_state = privileged_state.at[g1Utils.priv_base_lin_vel_idxs].set(jax.lax.stop_gradient(linvel))
        privileged_state = privileged_state.at[g1Utils.priv_h_idx].set(jax.lax.stop_gradient(root_height))



        wm_state = jp.zeros((g1Utils.wm_observation_size,))
        wm_state = wm_state.at[g1Utils.wm_gravity_idxs].set(gravity)
        wm_state = wm_state.at[g1Utils.wm_base_ang_vel_idxs].set(gyro)
        wm_state = wm_state.at[g1Utils.wm_q_idxs].set(joint_angles)
        wm_state = wm_state.at[g1Utils.wm_qd_idxs].set(joint_vel)
        wm_state = wm_state.at[g1Utils.wm_commands_idxs].set(jax.lax.stop_gradient(info["command"]))
        ...
        wm_state = wm_state.at[g1Utils.wm_last_action_idxs].set(jax.lax.stop_gradient(info["last_act"]))
        wm_state = wm_state.at[g1Utils.wm_quat_idxs].set(quat)
        wm_state = wm_state.at[g1Utils.wm_base_lin_vel_idxs].set(linvel)
        wm_state = wm_state.at[g1Utils.wm_h_idx].set(root_height)
        wm_state = wm_state.at[g1Utils.wm_gait_process_idx].set(jax.lax.stop_gradient(info["gait_process"]))
        wm_state = wm_state.at[g1Utils.wm_gait_frequency_idx].set(jax.lax.stop_gradient(info["gait_freq"]))
