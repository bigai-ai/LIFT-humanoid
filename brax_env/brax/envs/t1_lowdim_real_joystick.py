from brax.robots.booster.utils_real import BoosterUtils
...
class T1LowDimRealJoystick(RlwamEnv):
    """ T1LowDimRealJoystick environment
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        sys = BoosterUtils.get_system(used_cached_systems)
        ...
        # indices and limits live in BoosterUtils (no local copies)

        self.reward_config = BoosterUtils.reward_config()
    ...
    def reset(self, rng: jp.ndarray) -> State:
        ...
        q = q.at[BoosterUtils.xml_quat_idxs].set(initial_quat)
        ...
        last_last_actions = jp.zeros(BoosterUtils.action_size)
        last_actions = jp.zeros(BoosterUtils.action_size)
        actions = jp.zeros(BoosterUtils.action_size)
        info = {
        ...
        norm_actor_state, norm_critic_state = self._get_obs(
            pipeline_state,
            rew_info,
            jp.zeros(BoosterUtils.action_size),
            jp.zeros(BoosterUtils.action_size),
        )
        norm_wm_state = self._get_wm_state(
            pipeline_state,
            rew_info,
            jp.zeros(BoosterUtils.action_size),
            jp.zeros(BoosterUtils.action_size),
        ...
        u = jp.zeros(BoosterUtils.action_size)

        return State(pipeline_state, obs, reward, done, metrics,
                     info=info, u=u, rew_info=rew_info, torque=jp.zeros(BoosterUtils.action_size))
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        prev_norm_wm_state = self._get_wm_state(
        ...
        def f(info_list, _):
            ...
            torque, cmd = self.torque_pd_control(action, BoosterUtils.KP, BoosterUtils.KD, pipeline_state.q[BoosterUtils.xml_q_idxs], pipeline_state.qd[BoosterUtils.xml_qd_idxs])
        ...
        new_wm_state = BoosterUtils.denormalize_state(new_norm_wm_state, BoosterUtils.wm_state_limits)



        reward, rew_components = self.compute_reward(
            norm_wm_state=new_norm_wm_state,
            prev_norm_wm_state=prev_norm_wm_state,
            torques=torque,
            action=action,
            info=rew_info,
            valid_step=1.0,
        ...
        metrics.update(
            forward_vel=new_wm_state[BoosterUtils.wm_base_lin_vel_x_idx],
    ...
    def _is_healthy(self, next_norm_wm_state: jp.ndarray) -> jp.ndarray:
        ...
        next_wm_state = BoosterUtils.denormalize_state(next_norm_wm_state, BoosterUtils.wm_state_limits)
        quat = next_wm_state[BoosterUtils.wm_quat_idxs] / jp.linalg.norm(next_wm_state[BoosterUtils.wm_quat_idxs])
        roll, pitch, yaw = quat_to_eulerzyx(next_wm_state[BoosterUtils.wm_quat_idxs])

        base_vel_body = next_wm_state[BoosterUtils.wm_base_lin_vel_idxs]
        base_vel_global = rotate(base_vel_body, quat)
        ang_vel_body = next_wm_state[BoosterUtils.wm_base_ang_vel_idxs]
        ang_vel_global = rotate(ang_vel_body, quat)

        # Combine all conditions into one single condition using jp.logical_and:
        condition = next_wm_state[BoosterUtils.wm_h_idx] > BoosterUtils.terminate_height_min
        condition = jp.logical_and(condition, next_wm_state[BoosterUtils.wm_h_idx] < BoosterUtils.terminate_height_max)
        condition = jp.logical_and(condition, jp.all(jp.abs(base_vel_global) < BoosterUtils.terminate_lin_vel_max))
        condition = jp.logical_and(condition, jp.all(jp.abs(ang_vel_global) < BoosterUtils.terminate_ang_vel_max))
        condition = jp.logical_and(condition, jp.abs(roll) < BoosterUtils.fallen_roll)
        condition = jp.logical_and(condition, jp.abs(pitch) < BoosterUtils.fallen_pitch)
        condition = jp.logical_and(condition, jp.all(next_wm_state[BoosterUtils.wm_q_idxs] < BoosterUtils.UPPER_JOINT_LIMITS*1.5))
        condition = jp.logical_and(condition, jp.all(next_wm_state[BoosterUtils.wm_q_idxs] > BoosterUtils.LOWER_JOINT_LIMITS*1.5))
        condition = jp.logical_and(condition, jp.all(next_wm_state[BoosterUtils.wm_qd_idxs] < BoosterUtils.MOTOR_VEL_LIMIT*4))
        condition = jp.logical_and(condition, jp.all(next_wm_state[BoosterUtils.wm_qd_idxs] > -BoosterUtils.MOTOR_VEL_LIMIT*4))
    ...
    def dynamics_contact_integrate_only(
        ...
        wm_state = BoosterUtils.denormalize_state(norm_wm_state, BoosterUtils.wm_state_limits)
        wm_state_next = BoosterUtils.denormalize_state(norm_wm_state_next, BoosterUtils.wm_state_limits)
        ...
        def f(state_torque, _):
            ...
            torques, _ = self.torque_pd_control(desired_q,  BoosterUtils.KP, BoosterUtils.KD, state.q[BoosterUtils.xml_q_idxs], state.qd[BoosterUtils.xml_qd_idxs])
            torques = torques.astype(desired_q.dtype)
    ...
    def mbpo_dynamics(self, norm_obs: jp.ndarray, desired_q: jp.ndarray,
                      ext_forces: Optional[jp.ndarray] = None,
                      norm_obs_next: Optional[jp.ndarray] = None):
        norm_obs_new = norm_obs + ext_forces # TODO: clock may bug
        obs_next = BoosterUtils.denormalize_state(norm_obs_new, BoosterUtils.wm_state_limits)
        obs_prev = BoosterUtils.denormalize_state(norm_obs, BoosterUtils.wm_state_limits)
        q, qd = self.q_and_qd_from_wm_state(obs_next)
        ...
        state = mass.matrix_inv(sys, state, sys.matrix_inv_iterations)
        state = state.replace(constraint_forces=jp.zeros_like(state.con_diag))
        q = obs_prev[BoosterUtils.wm_q_idxs]
        qd = obs_prev[BoosterUtils.wm_qd_idxs]
        torques, _ = self.torque_pd_control(desired_q,  BoosterUtils.KP, BoosterUtils.KD, q, qd)
        return norm_obs_new, state.x.pos, state.xd.vel, state.xd.ang, state.x.rot, state.qdd, torques
    def q_and_qd_from_wm_state(self, obs: jp.ndarray):
        q = self.sys.init_q
        # normalize quat
        quat = obs[BoosterUtils.wm_quat_idxs] / jp.linalg.norm(obs[BoosterUtils.wm_quat_idxs])
        q = q.at[BoosterUtils.xml_quat_idxs].set(quat)
        q = q.at[BoosterUtils.xml_q_idxs].set(obs[BoosterUtils.wm_q_idxs])
        q = q.at[BoosterUtils.xml_h_idxs].set(obs[BoosterUtils.wm_h_idx])


        base_vel_body = obs[BoosterUtils.wm_base_lin_vel_idxs]
        base_vel_global = rotate(base_vel_body, quat)
        ang_vel_body = obs[BoosterUtils.wm_base_ang_vel_idxs]
        qd = jp.zeros(self.sys.qd_size())
        qd = qd.at[BoosterUtils.xml_base_vel_idxs].set(base_vel_global)
        qd = qd.at[BoosterUtils.xml_rpy_rate_idxs].set(ang_vel_body)
        qd = qd.at[BoosterUtils.xml_qd_idxs].set(obs[BoosterUtils.wm_qd_idxs])
    ...
    def make_ssrl_dynamics_fn(self, fn_type) -> Callable:
        ...
        def dynamics_fn(norm_obs: jp.ndarray, desired_q: jp.ndarray, pred: jp.ndarray):

            obs_next = BoosterUtils.denormalize_state(norm_obs, BoosterUtils.wm_state_limits)
            gait_process = obs_next[BoosterUtils.wm_gait_process_idx]
            gait_frequency = obs_next[BoosterUtils.wm_gait_frequency_idx]
            new_gait_process = jp.fmod(gait_process + self.sim_dt*self.policy_repeat * gait_frequency, 1.0)

            # TODO: COMMAND, 
            obs_next = obs_next.at[BoosterUtils.wm_cos_phase_idx].set(jp.cos(2*jp.pi*new_gait_process) * (gait_frequency > 1.0e-8).astype(new_gait_process.dtype) * (jp.linalg.norm(obs_next[BoosterUtils.wm_commands_idxs]) > 0.01).astype(new_gait_process.dtype))
            obs_next = obs_next.at[BoosterUtils.wm_sin_phase_idx].set(jp.sin(2*jp.pi*new_gait_process) * (gait_frequency > 1.0e-8).astype(new_gait_process.dtype) * (jp.linalg.norm(obs_next[BoosterUtils.wm_commands_idxs]) > 0.01).astype(new_gait_process.dtype))
            obs_next = obs_next.at[BoosterUtils.wm_gait_process_idx].set(new_gait_process)


            norm_obs_next = BoosterUtils.normalize_state(obs_next, BoosterUtils.wm_state_limits)


            norm_obs_new = fn(norm_obs, desired_q, pred, norm_obs_next)
            return norm_obs_new
    ...
    def _get_wm_state(
        ...
        foot_speed = xd.vel[BoosterUtils.feet_indices]
        ...
        p_fz = x.pos[BoosterUtils.feet_indices, 2]
        h = pipeline_state.q[BoosterUtils.xml_h_idxs] - jp.min(p_fz)

        wm_state = jp.zeros((BoosterUtils.wm_observation_size,))
        wm_state = wm_state.at[BoosterUtils.wm_gravity_idxs].set(projected_gravity)
        wm_state = wm_state.at[BoosterUtils.wm_base_ang_vel_idxs].set(ang_vel_body)
        wm_state = wm_state.at[BoosterUtils.wm_q_idxs].set(q)
        wm_state = wm_state.at[BoosterUtils.wm_qd_idxs].set(qd)
        wm_state = wm_state.at[BoosterUtils.wm_commands_idxs].set(jax.lax.stop_gradient(self.commands[:3]))
        wm_state = wm_state.at[BoosterUtils.wm_cos_phase_idx].set(jax.lax.stop_gradient(phase[0]))
        wm_state = wm_state.at[BoosterUtils.wm_sin_phase_idx].set(jax.lax.stop_gradient(phase[1]))
        wm_state = wm_state.at[BoosterUtils.wm_last_action_idxs].add(jax.lax.stop_gradient(last_action))
        wm_state = wm_state.at[BoosterUtils.wm_quat_idxs].set(quat)
        wm_state = wm_state.at[BoosterUtils.wm_base_lin_vel_idxs].set(base_vel_body)
        wm_state = wm_state.at[BoosterUtils.wm_h_idx].set(h)
        wm_state = wm_state.at[BoosterUtils.wm_gait_process_idx].set(jax.lax.stop_gradient(gait_process))
        wm_state = wm_state.at[BoosterUtils.wm_gait_frequency_idx].set(jax.lax.stop_gradient(gait_frequency))

        norm_wm_state = BoosterUtils.normalize_state(wm_state, BoosterUtils.wm_state_limits)
    ...
    def _get_obs(
        ...
        actor_state = jp.zeros((BoosterUtils.observation_size,))
        actor_state = actor_state.at[BoosterUtils.actor_gravity_idxs].add(jax.lax.stop_gradient(projected_gravity))
        actor_state = actor_state.at[BoosterUtils.actor_base_ang_vel_idxs].add(jax.lax.stop_gradient(ang_vel_body))
        actor_state = actor_state.at[BoosterUtils.actor_q_idxs].add(jax.lax.stop_gradient(q))
        actor_state = actor_state.at[BoosterUtils.actor_qd_idxs].add(jax.lax.stop_gradient(qd))
        actor_state = actor_state.at[BoosterUtils.actor_commands_idxs].set(jax.lax.stop_gradient(self.commands[:3]))
        actor_state = actor_state.at[BoosterUtils.actor_cos_phase_idx].set(jax.lax.stop_gradient(phase[0]))
        actor_state = actor_state.at[BoosterUtils.actor_sin_phase_idx].set(jax.lax.stop_gradient(phase[1]))
        actor_state = actor_state.at[BoosterUtils.actor_last_action_idxs].add(jax.lax.stop_gradient(last_action))

        privileged_state = jp.zeros((BoosterUtils.priv_observation_size,))
        privileged_state = privileged_state.at[BoosterUtils.priv_gravity_idxs].add(jax.lax.stop_gradient(projected_gravity))
        privileged_state = privileged_state.at[BoosterUtils.priv_base_ang_vel_idxs].add(jax.lax.stop_gradient(ang_vel_body))
        privileged_state = privileged_state.at[BoosterUtils.priv_q_idxs].add(jax.lax.stop_gradient(q))
        privileged_state = privileged_state.at[BoosterUtils.priv_qd_idxs].add(jax.lax.stop_gradient(qd))
        privileged_state = privileged_state.at[BoosterUtils.priv_commands_idxs].set(jax.lax.stop_gradient(self.commands[:3]))
        privileged_state = privileged_state.at[BoosterUtils.priv_cos_phase_idx].set(jax.lax.stop_gradient(phase[0]))
        privileged_state = privileged_state.at[BoosterUtils.priv_sin_phase_idx].set(jax.lax.stop_gradient(phase[1]))
        privileged_state = privileged_state.at[BoosterUtils.priv_last_action_idxs].add(jax.lax.stop_gradient(last_action))
        privileged_state = privileged_state.at[BoosterUtils.priv_quat_idxs].add(jax.lax.stop_gradient(quat))
        privileged_state = privileged_state.at[BoosterUtils.priv_base_lin_vel_idxs].add(jax.lax.stop_gradient(base_vel_body))
        privileged_state = privileged_state.at[BoosterUtils.priv_h_idx].set(jax.lax.stop_gradient(h))

        norm_actor_state = BoosterUtils.normalize_state(actor_state, BoosterUtils.actor_state_limits)
        norm_critic_state = BoosterUtils.normalize_state(privileged_state, BoosterUtils.priv_state_limits)
    ...
    def _get_obs_approx(
        ...
        #x, xd = kinematics.forward(self.sys, q_all, qd_all)
        #foot_speed = xd.vel[BoosterUtils.feet_indices]
        cos_phase = obs_next[BoosterUtils.wm_cos_phase_idx]
        sin_phase = obs_next[BoosterUtils.wm_sin_phase_idx]
        gait_process = obs_next[BoosterUtils.wm_gait_process_idx]
        gait_frequency = obs_next[BoosterUtils.wm_gait_frequency_idx]
        command = obs_next[BoosterUtils.wm_commands_idxs]
        phase = jp.array([cos_phase, sin_phase])


        projected_gravity = inv_rotate(jp.array([0.0, 0.0, -1.0]), quat)
        p_fz = pipeline_state.x.pos[BoosterUtils.feet_indices, 2]
        h = pipeline_state.q[BoosterUtils.xml_h_idxs] - jp.min(p_fz)
        actor_state = jp.zeros((BoosterUtils.observation_size,))
        actor_state = actor_state.at[BoosterUtils.actor_gravity_idxs].add(jax.lax.stop_gradient(projected_gravity))
        actor_state = actor_state.at[BoosterUtils.actor_base_ang_vel_idxs].add(jax.lax.stop_gradient(ang_vel_body))
        actor_state = actor_state.at[BoosterUtils.actor_q_idxs].add(jax.lax.stop_gradient(q))
        actor_state = actor_state.at[BoosterUtils.actor_qd_idxs].add(jax.lax.stop_gradient(qd))
        actor_state = actor_state.at[BoosterUtils.actor_commands_idxs].set(jax.lax.stop_gradient(command))
        actor_state = actor_state.at[BoosterUtils.actor_cos_phase_idx].set(jax.lax.stop_gradient(phase[0]))
        actor_state = actor_state.at[BoosterUtils.actor_sin_phase_idx].set(jax.lax.stop_gradient(phase[1]))
        actor_state = actor_state.at[BoosterUtils.actor_last_action_idxs].add(jax.lax.stop_gradient(last_action))
        ...
        privileged_state = privileged_state.at[BoosterUtils.priv_q_idxs].add(jax.lax.stop_gradient(q))
        privileged_state = privileged_state.at[BoosterUtils.priv_qd_idxs].add(jax.lax.stop_gradient(qd))
        privileged_state = privileged_state.at[BoosterUtils.priv_commands_idxs].set(jax.lax.stop_gradient(command))
        privileged_state = privileged_state.at[BoosterUtils.priv_cos_phase_idx].set(jax.lax.stop_gradient(phase[0]))
        privileged_state = privileged_state.at[BoosterUtils.priv_sin_phase_idx].set(jax.lax.stop_gradient(phase[1]))
        privileged_state = privileged_state.at[BoosterUtils.priv_last_action_idxs].add(jax.lax.stop_gradient(last_action))
        privileged_state = privileged_state.at[BoosterUtils.priv_quat_idxs].add(jax.lax.stop_gradient(quat))
        ...
        wm_state = wm_state.at[BoosterUtils.wm_q_idxs].set(q)
        wm_state = wm_state.at[BoosterUtils.wm_qd_idxs].set(qd)
        wm_state = wm_state.at[BoosterUtils.wm_commands_idxs].set(jax.lax.stop_gradient(command))
        wm_state = wm_state.at[BoosterUtils.wm_cos_phase_idx].set(jax.lax.stop_gradient(phase[0]))
        wm_state = wm_state.at[BoosterUtils.wm_sin_phase_idx].set(jax.lax.stop_gradient(phase[1]))
        wm_state = wm_state.at[BoosterUtils.wm_last_action_idxs].add(jax.lax.stop_gradient(last_action))
        wm_state = wm_state.at[BoosterUtils.wm_quat_idxs].set(quat)
    ...
    def _get_basic_obs(self, pipeline_state: base.State) -> jp.ndarray:
        ...
        quat = positions[BoosterUtils.xml_quat_idxs]

        # joint angles
        q = positions[BoosterUtils.xml_q_idxs]

        # linear velocity of the base in the body frame
        base_vel_global = velocities[BoosterUtils.xml_base_vel_idxs]
        base_vel_body = inv_rotate(base_vel_global, quat)

        # angular velocity of the base in the body frame
        ang_vel_body = velocities[BoosterUtils.xml_rpy_rate_idxs]

        # joint speeds
        qd = velocities[BoosterUtils.xml_qd_idxs]
    ...
    def compute_reward(self, norm_wm_state: jp.ndarray, prev_norm_wm_state: jp.ndarray,
        ...
        wm_state = BoosterUtils.denormalize_state(norm_wm_state, BoosterUtils.wm_state_limits)
        prev_wm_state = BoosterUtils.denormalize_state(prev_norm_wm_state, BoosterUtils.wm_state_limits)
    ...
    def _reward_normalized(self, wm_state: jp.ndarray,
        ...
        def do_main(args):
            ...
            gait_process = wm_state[BoosterUtils.wm_gait_process_idx]
            gait_frequency = wm_state[BoosterUtils.wm_gait_frequency_idx]
            ...
            quat = wm_state[BoosterUtils.wm_quat_idxs] / jp.linalg.norm(wm_state[BoosterUtils.wm_quat_idxs])

            base_lin_vel = wm_state[BoosterUtils.wm_base_lin_vel_idxs]
            base_ang_vel = wm_state[BoosterUtils.wm_base_ang_vel_idxs]
            base_euler_xyz = quat_to_euler(quat)
            projected_gravity = inv_rotate(jp.array([0.0, 0.0, -1.0]), quat)
            contact = rigid_state_pos[BoosterUtils.feet_indices, 2] < self.mini_ankle_dist

            commands = wm_state[BoosterUtils.wm_commands_idxs]

            feet_pos = rigid_state_pos[BoosterUtils.feet_indices, :3]
            feet_quat = rigid_state_rot[BoosterUtils.feet_indices, :4]
            # concat feet_quat
            ...
            reward_components = {
                ...
                "power": self._cost_energy(qvel=wm_state[BoosterUtils.wm_qd_idxs], torques=torques),
                "lin_vel_z": self._cost_lin_vel_z(base_lin_vel),
                "ang_vel_xy": self._cost_ang_vel_xy(base_ang_vel),
                "dof_vel": self._cost_dof_vel(wm_state[BoosterUtils.wm_qd_idxs]),
                "dof_acc": self._cost_dof_acc(rigid_state_qdd[6:]),
                "root_acc": self._cost_root_acc(rigid_state_qdd, quat),
                "action_rate": self._cost_action_rate(act=action, last_act=last_actions, last_last_act=last_last_actions),
                "dof_pos_limits": self._cost_joint_pos_limits(wm_state[BoosterUtils.wm_q_idxs]),
                "collision": self._cost_collision(left_foot_pos=feet_pos[0], right_foot_pos=feet_pos[1]),
                "feet_slip": self._cost_feet_slip(feet_vel=rigid_state_lin_vel[BoosterUtils.feet_indices], contact=contact),
                "feet_vel_z": self._cost_feet_vel_z(feet_vel=rigid_state_lin_vel[BoosterUtils.feet_indices]),
    ...
    def _reward_base_height(self, height: float) -> jax.Array:
        return jp.square(height - self.reward_config.base_height_target)
    ...
    def _cost_joint_pos_limits(self, qpos: jp.ndarray) -> jp.ndarray:
        below = qpos < BoosterUtils.soft_lowers
        above = qpos > BoosterUtils.soft_uppers
    ...
    def _cost_torque_tiredness(self, torques: jax.Array) -> jax.Array:
        ...
        frac = jp.clip(jp.abs(torques) / BoosterUtils.MOTOR_TORQUE_LIMIT, 0.0, 1.0)
        return jp.sum(jp.square(frac))

    def _cost_root_acc(self, qdd: jax.Array, quat: jax.Array) -> jax.Array:
        # Root‑link 6‑D acceleration² (free‑joint entries of qacc)

        # linear acc of the base in the body frame
        base_linacc_global = qdd[:3]
        lin_acc_b = inv_rotate(base_linacc_global, quat)

        # angular acc of the base in the body frame
        ang_acc_b = qdd[3:6]

        return jp.vdot(ang_acc_b, ang_acc_b) + jp.vdot(lin_acc_b, lin_acc_b)
    ...
    def action_size(self) -> int:
        return BoosterUtils.action_size
    ...
    def observation_size(self) -> int:
        return {
            "state": BoosterUtils.observation_size,
            "privileged_state": BoosterUtils.priv_observation_size,
            "wm_state": BoosterUtils.wm_observation_size,
        }
