...
class Ant2(PipelineEnv):
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        state.metrics.update(
            ...
            reward_survive=rm['reward_survive'],
            reward_ctrl=rm['reward_ctrl'],
            reward_contact=rm['reward_contact'],
            x_position=pipeline_state.x.pos[0, 0],
            y_position=pipeline_state.x.pos[0, 1],
            distance_from_origin=math.safe_norm(pipeline_state.x.pos[0]),
            x_velocity=rm['x_velocity'],
            y_velocity=rm['y_velocity'],
            forward_reward=rm['forward_reward'],
        )
    ...
    def compute_reward(self, obs: jp.ndarray, obs_next: jp.ndarray,
        ...
        if self._exclude_current_positions_from_observation:
            velocity = obs_next[13:16]
        else:
            velocity = obs_next[15:18]
