...
def eval_wrap(
    ...
    env = FrameStackWrapper(env, obs_history_length, priv_obs_history_length)
    env = EpisodeNoResetWrapper(env, episode_length, action_repeat)
...
def wrap(
    ...
    env = FrameStackWrapper(env, obs_history_length, priv_obs_history_length, wm_obs_history_length)
...
class FrameStackWrapper(Wrapper):
    ...
    def __init__(self, env: Env, obs_history_length: int, priv_obs_history_length: int, wm_obs_history_length: int,noise_std: jp.ndarray = 0.0):
        super().__init__(env)
        self.obs_history_length = obs_history_length
        self.priv_obs_history_length = priv_obs_history_length
        self.wm_obs_history_length = wm_obs_history_length
    ...
    def reset(self, rng: jp.ndarray) -> State:
        ...
        obs_stack = {
            'state': jp.tile(nstate.obs['state'], self.obs_history_length),
            'privileged_state': jp.tile(nstate.obs['privileged_state'], self.priv_obs_history_length),
            'wm_state': jp.tile(nstate.obs['wm_state'], self.wm_obs_history_length),
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        obs_old = { 
          'state': state.obs['state'][:self._unwrapped_obs_size['state']],
          'privileged_state': state.obs['privileged_state'][:self._unwrapped_obs_size['privileged_state']],
          'wm_state': state.obs['wm_state'][:self._unwrapped_obs_size['wm_state']]
        ...
        obs_stack = jp.concatenate(
          [nstate.obs['state'], state.obs['state'][:self._unwrapped_obs_size['state']*(self.obs_history_length-1)]],
          axis=-1
        )

        priv_obs_stack = jp.concatenate(
          [nstate.obs['privileged_state'], state.obs['privileged_state'][:self._unwrapped_obs_size['privileged_state']*(self.priv_obs_history_length-1)]],
          axis=-1
        )

        if self.wm_obs_history_length == 1:
            wm_obs_stack = nstate.obs['wm_state']
        else:
            wm_obs_stack = jp.concatenate(
              [nstate.obs['wm_state'],
               state.obs['wm_state'][:self._unwrapped_obs_size['wm_state']*(self.wm_obs_history_length-1)]],
              axis=0
            )
        ...
        return nstate.replace(obs=obs, prev_obs=state.obs)


    def low_level_control(self, action: jp.ndarray,
                          obs: jp.ndarray) -> jp.ndarray:
        return self.env.low_level_control(action, obs['privileged_state'][:self._unwrapped_obs_size['privileged_state']])

    def compute_reward(self, obs: jp.ndarray, prev_obs: jp.ndarray,
                       u: jp.ndarray, unscaled_action: jp.ndarray) -> jp.ndarray:
        return self.env.compute_reward(obs['privileged_state'][:self._unwrapped_obs_size['privileged_state']],
                                       prev_obs['privileged_state'][:self._unwrapped_obs_size['privileged_state']],
                                       u, unscaled_action)

    @property
    def observation_size(self) -> int:
        return {
          'state': self._unwrapped_obs_size['state'] * self.obs_history_length,
          'privileged_state': self._unwrapped_obs_size['privileged_state'] * self.priv_obs_history_length,
          'wm_state': self._unwrapped_obs_size['wm_state'] * self.wm_obs_history_length
        }
...
class EpisodeNoResetWrapper(Wrapper):
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        state.info['episode_metrics']['sum_reward'] += jp.sum(rewards, axis=0) * (1 - prev_done)
        state.info['episode_metrics']['length'] += self.action_repeat * (1 - prev_done)
        for metric_name in state.metrics.keys():
            if metric_name != 'reward':
                state.info['episode_metrics'][metric_name] += state.metrics[metric_name] * (1 - prev_done)
...
class AutoResetWrapper(Wrapper):
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        state.info['first_rew_info'] = state.rew_info
        return state
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        rew_info = jax.tree.map(where_done, state.info['first_rew_info'], state.rew_info)
        metrics = jax.tree.map(
            where_done, state.info['first_metrics'], state.metrics
        )
        return state.replace(
          pipeline_state=pipeline_state, obs=obs, metrics=metrics, rew_info=rew_info
        )
