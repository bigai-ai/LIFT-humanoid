from brax.robots.booster.utils_sim import BoosterUtils
...
class T1LowDimSimJoystick(RlwamEnv):
    """ T1LowDimSimJoystick environment"""
    ...
    def reset(self, rng: jp.ndarray) -> State:
        ...
        norm_wm_state = self._get_wm_state(pipeline_state, rew_info, jp.zeros(BoosterUtils.action_size), jp.zeros(BoosterUtils.action_size))
        # compute cmd for info
        _, rew_components = self.compute_reward(
            norm_wm_state, norm_wm_state, jp.zeros(BoosterUtils.action_size), jp.zeros(BoosterUtils.action_size), rew_info, 0.0)
    ...
    def mbpo_dynamics(self, norm_obs: jp.ndarray, desired_q: jp.ndarray,
        ...
        obs_prev = BoosterUtils.denormalize_state(norm_obs, BoosterUtils.wm_state_limits)        # get q and qd from obs
    ...
    def make_ssrl_dynamics_fn(self, fn_type) -> Callable:
        ...
        def dynamics_fn(norm_wm_state: jp.ndarray, desired_q: jp.ndarray, pred: jp.ndarray):
            ...
            # TODO: COMMAND, 
            wm_state = wm_state.at[BoosterUtils.wm_cos_phase_idx].set(jp.cos(2*jp.pi*new_gait_process) * (gait_frequency > 1.0e-8).astype(new_gait_process.dtype) * (jp.linalg.norm(wm_state[BoosterUtils.wm_commands_idxs]) > 0.01).astype(new_gait_process.dtype))
            wm_state = wm_state.at[BoosterUtils.wm_sin_phase_idx].set(jp.sin(2*jp.pi*new_gait_process) * (gait_frequency > 1.0e-8).astype(new_gait_process.dtype) * (jp.linalg.norm(wm_state[BoosterUtils.wm_commands_idxs]) > 0.01).astype(new_gait_process.dtype))
            wm_state = wm_state.at[BoosterUtils.wm_gait_process_idx].set(new_gait_process)


            norm_wm_state_next = BoosterUtils.normalize_state(wm_state, BoosterUtils.wm_state_limits)
    ...
    def _get_wm_state(self, pipeline_state: base.State,
        ...
        h = pipeline_state.q[BoosterUtils.xml_h_idxs]
    ...
    def _get_obs_approx(self, pipeline_state: base.State,
        ...
        #x, xd = kinematics.forward(self.sys, q_all, qd_all)
        #foot_speed = xd.vel[BoosterUtils.feet_indices]
        cos_phase = wm_state_next[BoosterUtils.wm_cos_phase_idx]
        sin_phase = wm_state_next[BoosterUtils.wm_sin_phase_idx]
        gait_process = wm_state_next[BoosterUtils.wm_gait_process_idx]
        gait_frequency = wm_state_next[BoosterUtils.wm_gait_frequency_idx]
        command = wm_state_next[BoosterUtils.wm_commands_idxs]
        phase = jp.array([cos_phase, sin_phase])
