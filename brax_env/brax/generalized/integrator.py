...
def integrate(sys: System, state: State) -> State:
    ...
    #qdd = state.qf_constraint#mx_inv @ (state.qf_smooth + state.qf_constraint)
...
def integrate_euler(sys: System, state: State) -> State:
    ...
    q = scan.link_types(sys, q_fn, 'lqd', 'q', sys.link, state.q, state.qd)

    return state.replace(q=q, qd=qd, qdd=qdd)


def integrate_qdd(sys: System, state: State, qdd: jp.ndarray) -> State:
    """Semi-implicit Euler integration if qdd is givem.

    Args:
      sys: system defining the kinematic tree and other properties
      state: generalized state
      qdd: generalized acceleration
