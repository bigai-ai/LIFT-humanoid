...
def jac_contact(
    ...
    def row_fn(contact):
        ...
        return jax.tree_map(
            lambda x: x * (jp.logical_or(contact.penetration > 0, ignore_penetration)), (jac, diag, aref),
        )

    return jax.tree_map(jp.concatenate, jax.vmap(row_fn)(contact))


def jacobian(sys: System, state: State,
             ignore_penetration: bool = False) -> State:
    ...
    """
    jpds = (jac_contact(sys, state, ignore_penetration=ignore_penetration),
            jac_limit(sys, state))
...
def force(sys: System, state: State) -> jp.ndarray:
    ...
    if state.con_jac.shape[0] == 0:
        return jp.zeros(sys.qd_size()), jp.zeros_like(state.con_diag)
    ...
    # solve forces
    forces = pg.run(jp.zeros_like(b)).params

    # convert back to q coordinates
    qf_constraint = state.con_jac.T @ forces

    return qf_constraint, forces
