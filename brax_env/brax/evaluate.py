...
from brax_env.brax_env_utils import Evaluator
...
def evaluate(params, env_unwrapped: envs.Env,
    ...
    env = FrameStackWrapper(env_unwrapped, obs_history_length=obs_history_length, priv_obs_history_length=priv_obs_history_length, wm_obs_history_length=wm_obs_history_length)
    ...
    for _ in range(episode_length // action_repeat):
        ...
        for _ in range(action_repeat):
            ...
            obs.append(env_state.obs['privileged_state'][:len(env_state.obs['privileged_state'])//priv_obs_history_length])
            prev_obs.append(env_state.prev_obs['privileged_state'][:len(env_state.prev_obs['privileged_state'])//priv_obs_history_length])
