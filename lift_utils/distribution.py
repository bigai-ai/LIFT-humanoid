...
class ParametricDistribution(abc.ABC):
    ...
    def sample(self, parameters, seed):
        ...
        action_raw = self.sample_no_postprocessing(parameters, seed)
        action_tanh = self.postprocess(action_raw)
        return action_tanh
    ...
    def mode_no_postprocessing(self, parameters):
        """Returns the mode of the distribution."""
        return self.create_dist(parameters).mode()

    def stddev_no_postprocessing(self, parameters):
        """Returns the standard deviation of the distribution."""
        return self.create_dist(parameters).stddev()

    def log_prob(self, parameters, actions_raw):
        """Compute the log probability of actions."""
        dist = self.create_dist(parameters)
        log_probs = dist.log_prob(actions_raw)
        log_probs -= self._postprocessor.forward_log_det_jacobian(actions_raw)
...
class NormalTanhDistribution(ParametricDistribution):
    ...
    def __init__(self, event_size, min_log_std=-5.0, max_log_std=2.0):
    ...
    def create_dist(self, parameters):
        ...
        log_scale = self._min_log_std + 0.5 * (self._max_log_std - self._min_log_std) * (log_scale + 1)
